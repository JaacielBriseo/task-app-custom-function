...
const TOGGLE_TASK_STATUS_MUTATION = `
mutation TaskUpdate($taskId: ID!, $completed: Boolean!) {
  taskUpdate(filter: { id: $taskId }, data: { completed: $completed}) {
    id
    createdAt
    description
    completed
...
export default async (event: any, ctx: any) => {
	const { taskId, completed } = JSON.parse(event.body);
	// console.log({ taskId, completed });
	await ctx.api.gqlRequest(
		TOGGLE_TASK_STATUS_MUTATION,
		{
			taskId,
			completed,
		},
	...
	return {
		statusCode: 200,
		body: JSON.stringify({
			result: `Webhook received: ${taskId} - ${completed}`,
		}),
	};
};
